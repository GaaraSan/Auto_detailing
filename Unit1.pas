...
Except
        ShowMessage('Введите целое число в поле "Цена за единицу"');
end;
end;
...
DataModule2.ADOQServices.Open;
        ...
        ShowMessage('Введите целое число в поле "Поиск по коду товара"');
end;
end;
...
DataModule2.ADOQOrder.SQL.Text:= 'SELECT Order.ID AS ID , Services.Title, Order.Кількість_послуг, Check.ID AS Check_id, Order.Вартість '
+' FROM [Check] INNER JOIN (Staff INNER JOIN (Services INNER JOIN (Busyness INNER JOIN [Order] ON Busyness.ID = Order.Послуга) ON Services.ID = Busyness.service) ON Staff.ID = Busyness.staff) ON Check.ID = Order.Код_чека '
+' ORDER BY Order.ID';
...
DataModule2.ADOQOrder.Open;
        ...
        ShowMessage('Введите целое число в поле "Поиск по номеру чека');
end;
end;
